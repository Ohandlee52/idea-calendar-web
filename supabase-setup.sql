...
drop policy if exists "내 메모만 조회" on public.memos;
create policy "내 메모만 조회" on public.memos
  for select using (auth.uid() = user_id);

drop policy if exists "내 메모만 추가" on public.memos;
create policy "내 메모만 추가" on public.memos
  for insert with check (auth.uid() = user_id);

drop policy if exists "내 메모만 수정" on public.memos;
create policy "내 메모만 수정" on public.memos
  for update using (auth.uid() = user_id) with check (auth.uid() = user_id);

drop policy if exists "내 메모만 삭제" on public.memos;
create policy "내 메모만 삭제" on public.memos
  for delete using (auth.uid() = user_id);

-- 3) 빠른 조회를 위한 색인 -----------------------------------------
create index if not exists memos_user_date_idx    on public.memos (user_id, date);
